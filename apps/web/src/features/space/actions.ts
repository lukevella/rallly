...
export const deleteSpaceAction = spaceActionClient
  ...
  .action(async ({ ctx, parsedInput }) => {
    ...
    // Check if user has other spaces - can't delete last space
    const userSpaceCount = await prisma.spaceMember.count({
      where: {
        userId: ctx.user.id,
      },
    });
    ...
    if (userSpaceCount <= 1) {
      throw new AppError({
        code: "FORBIDDEN",
        message: "Cannot delete your last remaining space",
      });
    }
