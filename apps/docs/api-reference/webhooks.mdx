...
Webhooks are available to spaces on a Pro plan of the cloud hosted version. Only the space owner can manage endpoints, from **Settings → Webhooks**. They are not available on self-hosted instances.

## Adding an endpoint

1. Open **Settings → Webhooks** and choose **Add webhook**.
...
See [Events](/api-reference/webhooks/events) for the envelope and headers, and [Poll events](/api-reference/webhooks/events/poll) for every event with its payload.
