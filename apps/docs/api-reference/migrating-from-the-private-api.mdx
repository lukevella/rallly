...
| Poll shape | `dates` or `slots` in the request, `options` in the response | `kind` plus `options` on both sides, so a `GET` body can be sent back to `POST` |
...
Replace the host and prefix. Paths under it are the same.
...
**Request.** The private API chose the poll type from whether you sent `dates` or `slots`. v1 takes an explicit `kind` and puts every option under `options`, in the same shape the response returns.
