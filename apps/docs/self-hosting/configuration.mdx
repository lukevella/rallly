...
<ParamField path="SMTP_TLS_SERVERNAME">
  Explicitly sets the server name for SNI (Server Name Indication) during the TLS handshake. Only needed for SMTP servers that host multiple certificates and require SNI to return the correct one. In most cases this is not required.
