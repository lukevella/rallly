...
<ParamField path="SMTP_DEBUG">
  Removed in v4.16.0 and ignored if set. It wrote every recipient address and the base64 encoded credentials to the log. The send failure log now carries the SMTP server's reply (`errorResponse`, `errorResponseCode`, `errorCommand`), which covers the cases it was added for. For connection level diagnosis run `openssl s_client -starttls smtp -connect host:587` against your server.
</ParamField>
