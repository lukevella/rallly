...
<ParamField path="SMTP_DEBUG" default="false">
  Set to `true` to log the full SMTP conversation. For development and local troubleshooting only.

  <Warning>Debug output includes the authentication exchange, where credentials are only base64 encoded, and every recipient address in the `RCPT TO` envelope. Do not enable it on an instance that sends mail to real users: the addresses land in your log storage and inherit its retention. If you must use it against a live server, do so briefly, then delete the affected logs and redact anything you share.</Warning>
