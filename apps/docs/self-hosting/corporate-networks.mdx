...
Rallly makes outbound requests to these hosts, all over HTTPS on port 443. If your firewall filters by destination, allow them.

| Host | Purpose | When blocked |
|:-----|:--------|:-------------|
| `licensing.rallly.co` | License activation and refresh | Activation fails with an error in the Control Panel |
| `api.rallly.co` | Hourly update check: sends the running version and an anonymous instance identifier, receives the latest release and any security advisories affecting your version | Fails silently. The version tile shows no status and you are not told about security updates, so watch the [releases page](https://github.com/lukevella/rallly/releases) instead |
