...
```sh
# Drop the freshly-migrated empty database and recreate it
docker compose exec -T db psql -U postgres -c "DROP DATABASE rallly;"
docker compose exec -T db psql -U postgres -c "CREATE DATABASE rallly;"

# Restore
gunzip < /path/to/rallly-backup.sql.gz | docker compose exec -T db psql -U postgres rallly
```

Restart the app so it picks up the restored schema:
...
<Warning>
  Double-check volume names before removing. The new stack's volumes live under `rallly_` (e.g. `rallly_db-data`, `rallly_garage-data`) — do not delete those.
</Warning>
