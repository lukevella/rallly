...
<ParamField path="OIDC_NAME_CLAIM_PATH" default="name">
  The JSON path in the OIDC claim where the user's name can be found.
</ParamField>

<ParamField path="OIDC_EMAIL_CLAIM_PATH" default="email">
  The JSON path in the OIDC claim where the user's email can be found.
</ParamField>

<ParamField path="OIDC_PICTURE_CLAIM_PATH" default="picture">
  The JSON path in the OIDC claim where the user's profile picture can be found.
</ParamField>

<Info>Use dot notation in `_PATH` fields to access nested objects.</Info>
