...
```sh
gunzip < backups/rallly_20260101_120000.sql.gz | docker compose exec -T db psql -U postgres rallly
```
...
Or a single service (`rallly`, `db`, `redis`, `garage`, `traefik`, `serverless-redis-http`):
